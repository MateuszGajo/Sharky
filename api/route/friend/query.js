...
with userFriends as(
select user_id_1 as "users" from friends where CASE when $4 then user_id_2=$1 else user_id_2=$1   end
union
select user_id_2 as "users" from friends where CASE when $4 then  user_id_1=$1 else user_id_1=$1   end 
),

userFriendsCounted as(
select a."userId", sum(a.count) as "numberOfFriends"	  
  from( select user_id_1 as "userId",count(user_id_1)  from friends where user_id_1 in(select * from userFriends) and status='1'  group by user_id_1
      union
      select user_id_2 as "userId",count(user_id_2)  from friends where user_id_2 in(select * from userFriends) and status='1'  group by user_id_2) as a
group by a."userId"
),

friendsStatus as(
select user_id_1 as "userId",id as "friendshipId",status,date, CASE WHEN status='0'  THEN true  else null end as "isInvited", null as "isInvitationSent" from friends where user_id_1 in(select * from userFriends) and user_id_2 =$2
union
select user_id_2 as "userId",id as "friendshipId",status,date,null as "isInvited",CASE WHEN status='0'  THEN true  else null end as "isInvitationSent" from friends where user_id_2 in(select * from userFriends) and user_id_1=$2
)
...
from(select a.*,coalesce(b."numberOfFriends",0) as "numberOfFriends", c.relation 
  ...
  left join userFriendsCounted as b on a."userId" = b."userId"
  left join friend_relations as c on a."friendshipId"= c.friendship_id
  union
  select a."userId",null as  friendshipId,null as  status,null as  date,null as "isInvited",null as "isInvitationSent",a."numberOfFriends",null as  relation from userFriendsCounted as a where a."userId" not in (select "userId" from friendsStatus)) as d
